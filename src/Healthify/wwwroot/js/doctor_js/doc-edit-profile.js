...
const load = async () => {
    ...
    document.getElementById("user-img-change").src = profileData.profilePicture;
...
const checkCountry = () => {
    ...
    if (country.value == '') {
        country.style.border = "1px solid #b4b4be";
        return true;
    }
    if (!reg1.test(country.value)) {
...
const postProfile = async () => {
    ...
    if (new Date(profileData.birthDate).toDateString() != new Date(dob).toDateString()) {
        post.push({
