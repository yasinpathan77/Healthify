const load = async () => {
    ...
    data.forEach(element => {
        ...
        if (element.profilePicture != null) {
            x +=
                `<img width="28" height="28" src="${element.profilePicture}" class="rounded-circle" alt=""> <h2> ${element.fname} ${element.lname} </h2>`;
        }
        else {
            x += `<img width="28" height="28" src="assets/img/user.jpg" class="rounded-circle" alt=""> <h2> ${element.fname} ${element.lname} </h2>`;
        }
        x += 
        `</td>
        <td> ${element.id} </td>
